...
hurdle_tempKBDI_pred <- predict(hurdle_tempKBDI, type = "response")
hurdle_TempOnly_pred <- predict(hurdle_TempOnly, type = "response")
...
presence_comparison <- data.frame(
  Model = c("Logistic", "Hurdle", "ZINB"),
  AUC = c(
    auc(your_data$Presence, logistic_pred),
    auc(your_data$Presence, hurdle_presence_pred),
    auc(your_data$Presence, zinb_presence_pred)
  )
)
...
zinb_Temp  <- zeroinfl(
  Count ~ Temp |  # Main model
    Temp,                      # Zero-inflation model
  data = ptaE,
  dist = "negbin"  # Negative binomial distribution
) 
...
PA_comparison <- data.frame(
  Model = c(logistic_Temp, 
            logistic_Temp_KBDI,
            zinb_Temp,
            zinb_temp_KBDI,
            hurdle_TempOnly,
            hurdle_tempKBDI),
  AUC = c(
    # Calculate AUC for each model's presence predictions
    auc(ptaE$Presence, predict(logistic_Temp, type = "response")),
    auc(ptaE$Presence, predict(logistic_Temp_KBDI, type = "response")),
    auc(ptaE$Presence, predict(zinb_temp_KBDI, type = "zero")),
    auc(ptaE$Presence, predict(zinb_Temp, type = "zero")),
    auc(ptaE$Presence, predict(hurdle_tempKBDI, type = "response")),
    auc(ptaE$Presence, predict(hurdle_TempOnly, type = "response"))
  )
)
...
PA_comparison <- data.frame(
  ...
  AUC = c(
    # Calculate AUC for each model's presence predictions
    auc(fire_data$fire_present, predict(logistic_model, type = "response")),
    auc(fire_data$fire_present, predict(hurdle_model, type = "response")),
    auc(fire_data$fire_present, predict(zinb_model, type = "zero"))
  )
)
